...
export async function validateSessionToken(token) {
	...
	const row = await db.queryOne(
		"SELECT user_sessions.id, user_sessions.user_id, user_sessions.expires_at, user.id FROM user_sessions INNER JOIN user ON user.id = user_sessions.user_id WHERE id = ?",
		sessionId
	);
	if (row === null) {
